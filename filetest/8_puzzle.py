import random
import itertools #untuk looping yg efisien
import collections #container
import time #untuk waktu running
...
class Node:
    ...
    def path(self):
        ...
        while node:
            p.append(node)
            node = node.parent
        yield from reversed(p) #yield=sama kaia return=mengembalikan nilai
                                #reversed=mbalik (aku=uak)
...
class Solver:
    ...
    def solve(self):
        ...
        Queue = FIFO
        """
        queue = collections.deque([Node(self.start)]) #deque=menghapus elemen terdepan
        seen = set()
        seen.add(queue[0].state)
        while queue:
            queue = collections.deque(sorted(list(queue), key=lambda node: node.f))#key=lambda adalah fungsi menggantikan inisiaisasi
            node = queue.popleft()
            if node.solved:
                return node.path
...
# example of use     
board = [[1,2,3,4],[5,6,0,8],[9,10,11,12],[13,14,15,7]]
puzzle = Puzzle(board)
